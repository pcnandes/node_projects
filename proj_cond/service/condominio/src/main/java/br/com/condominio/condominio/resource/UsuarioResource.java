...
public class UsuarioResource {
	...
	public ResponseEntity<Usuario> alterar(@PathVariable Long id, @RequestBody Usuario usuario) {
		...
		Usuario usuarioParaAtualizar = Usuario.builder().id(id).apelido(usuario.getApelido()).celular1(usuario.getCelular1())
				.celular2(usuario.getCelular2()).cpf(usuario.getCpf()).email(usuario.getEmail()).nome(usuario.getNome())
				.perfis(usuario.getPerfis()).senha(usuario.getSenha()).telefone(usuario.getTelefone()).build();
